...
public:
	...
	hex_file (string filename)
	{
		...
		if (read_hex_file () != 0)
		{
			std::cout << "Error opening HEX File: " << filename << std::endl;
			file->close ();
			exit (-1);
		}
	}
	...
	int
	copy_data_to_memory (unsigned char * data_out, int memlen)
	{
...
#ifdef TEST
					cout << "Copy " << (int) act_rec->data[addr_cnt] << " to Address : "
							<< act_rec->Address + addr_cnt << endl;
#endif
				}
			}
			else
...
private:
	...
	read_hex_file ()
	{
		...
		if (file->is_open ())
		{
			std::cout << "Hex file was opened successfully!" << std::endl;
			while (!file->eof ())
			{
				string line;
				getline (*file, line);
				...
				if (new_rec != NULL)
				{
					if (new_rec->Record_type == record::e_types::DATA)
